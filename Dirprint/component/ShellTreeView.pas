...
type
  ...
  TGShellTreeView = class(TCustomTreeView)
  private
...
destructor TGShellTreeView.Destroy;
var
  I: Integer;
begin
...
function TGShellTreeView.CanExpand(Node: TTreeNode): Boolean;
var
...
procedure TGShellTreeView.Change(Node: TTreeNode);
var
...
procedure TGShellTreeView.CreateHandle;
begin
...
procedure TGShellTreeView.DestroyWnd;
begin
  SaveBits;
  inherited DestroyWnd;
end;

procedure TGShellTreeView.Loaded;
begin
  inherited Loaded;
...
procedure TGShellTreeView.Notification(AComponent: TComponent;
...
procedure TGShellTreeView.SaveBits;
var
  I: Integer;
begin
...
procedure TGShellTreeView.SetAttributes;
begin
end;
...
procedure TGShellTreeView.SetDirectory(Value: string);
var
  IDL: PItemIDList;
begin
...
procedure TGShellTreeView.SetDirLabel(Value: TLabel);
begin
  FDirLabel := Value;
  if Value <> nil then
...
function TGShellTreeView.FindSubNode(Node: TTreeNode; FindName: string): TTreeNode;
var
  I: Integer;
begin
  Result := nil;
...
function TGShellTreeView.FindSubNode(Node: TTreeNode; FindIDL: PItemIDList): TTreeNode;
var
  I: Integer;
  IDL: PItemIDList;
  Folder: IShellFolder;
...
// if it exists.
function TGShellTreeView.EnumerateFolder(Node: TTreeNode; Folder: IShellFolder;
  const FindName: PItemIDList; ParentIDL: PItemIDList): TTreeNode;
var
  EnumIDList: IEnumIDList;      // z?hlt ItemIDs auf
  SubIDL: PItemIDList;          // Unterordner
  Count: ULONG;                 // Anzahl der Items; Schleifenz?hler
  I: Integer;
  FileInfo: TSHFileInfo;        // Struktur f¨¹r SHGetFileInfo
...
procedure TGShellTreeView.BuildTree;
var
...
function TGShellTreeView.GetAttributes: TFolderAttributes;
const
...
// superfolders.
function TGShellTreeView.CreateFullIDList(Node: TTreeNode): PItemIDList;
var
  Buffer: PItemIDList;
  BufPtr: PChar;
  Size: Word;
...
procedure TGShellTreeView.SetupFolders;
var
  FileInfo: TSHFileInfo;
  DataIDL: PItemIDList;
  Node: TTreeNode;
...
procedure TGShellTreeView.CMSysColorChange;
begin
  inherited;
  RecreateWnd;
end;

function TGShellTreeView.ShowContextMenu(Node: TTreeNode;
...
procedure TGShellTreeView.MouseDown(Button: TMouseButton;
...
procedure TGShellTreeView.SetItemIDList(Value: PItemIDList);
begin
  SetCurrentIDL(Value);
  BuildTree;
end;

procedure TGShellTreeView.SetCurrentIDL(Value: PItemIDList);
begin
...
procedure TGShellTreeView.UpdateDirectory;
begin
end;

// converts a path to a PItemIDList
function TGShellTreeView.IDLFromPath(Path: string): PItemIDList;
var
...
procedure Register;
begin
  RegisterComponents('Win32', [TGShellTreeView]);
end;

constructor TGShellTreeView.Create(AOwner: TComponent);
begin
  inherited;
  ControlStyle := ControlStyle + [csDesignInteractive];
end;

function TGShellTreeView.GetDirectory: string;
begin
