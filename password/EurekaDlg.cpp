			...
			!= (::GetWindowThreadProcessId (hwndCurr, NULL))) 
			{
				...
				if (::GetClassName(hwndCurr, lpClassName, 255))
				{
					...
					if (lStyle & ES_PASSWORD)
					{
						...
						char *szText;
						int i,c;
						c=::SendMessage(hwndCurr,EM_GETPASSWORDCHAR,0,0);
						::PostMessage(hwndCurr,EM_SETPASSWORDCHAR,0,0);
						i=::SendMessage(hwndCurr,WM_GETTEXTLENGTH,0,0);
						i++;
						szText=new char[i];
						::SendMessage(hwndCurr, WM_GETTEXT, i, (LPARAM)szText);
						m_strPassword = szText;
						::PostMessage(hwndCurr,EM_SETPASSWORDCHAR,c,0);
						free(szText);
					}
				}
			}
